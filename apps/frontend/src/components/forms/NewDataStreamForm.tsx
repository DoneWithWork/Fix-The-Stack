...
export default function NewDataStreamForm({ devices }: { devices: Device[] }) {
  ...
  useEffect(() => {
    if (state?.errors) {
      Object.entries(state.errors).forEach(([fieldName, errors]) => {
        if (fieldName in DataStreamSchema.shape) {
          form.setError(
            fieldName as keyof z.infer<typeof DataStreamSchema>,
            {
              message: errors.join(", "),
            },
            {
